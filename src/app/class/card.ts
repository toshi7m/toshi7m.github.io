...
export class Card extends TabletopObject {
  ...
  get godName(): string {
    let object = PeerCursor.findByUserId(this.god);
    return object ? object.name : '';
  }

  get hasOwner(): boolean { return 0 < this.owner.length; }
  get hasGod(): boolean { return 0 < this.god.length; }
  get isHand(): boolean { return Network.peerContext.userId === this.owner; }
  get isGod(): boolean { return Network.peerContext.userId === this.god; }
  get isFront(): boolean { return this.state === CardState.FRONT; }
  get isVisible(): boolean { return this.isHand || this.isFront || this.isGod; }
  ...
  godeye_start() {
    this.god = Network.peerContext.userId;
  }
