...
export class CardStackComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  get hasGod(): boolean { return this.cardStack.hasGod; }
  get godName(): string { return this.cardStack.godName; }
  ...
  onContextMenu(e: Event) {
    ...
    this.contextMenuService.open(position, [
      ...
      ContextMenuSeparator,
      {
        name: '配信モードオン', action: () => {
          SoundEffect.play(PresetSound.cardDraw);
          this.cardStack.godeye_start_All();
        }
      },
      {
        name: '配信モードオフ', action: () => {
          SoundEffect.play(PresetSound.cardDraw);
          this.cardStack.godeye_end_All();
        }
      },
      ContextMenuSeparator,
      {
