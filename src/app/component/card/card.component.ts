...
export class CardComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  get god(): string { return this.card.god; }
  set owner(owner: string) { this.card.owner = owner; }
  set god(god: string) { this.card.god = god; }
  ...
  get hasGod(): boolean { return this.card.hasGod; }
  get ownerName(): string { return this.card.ownerName; }
  get godName(): string { return this.card.godName; }
    ...
    EventSystem.register(this)
      .on('UPDATE_GAME_OBJECT', -1000, event => {
        ...
        if ((this.card === object)
          ...
          || (object instanceof PeerCursor && object.userId === this.card.god)) {
  ...
  onContextMenu(e: Event) {
    ...
    this.contextMenuService.open(position, [
      ...
      ContextMenuSeparator,
      {
        name: '配信モードオン', action: () => {
          SoundEffect.play(PresetSound.cardDraw);
          this.card.godeye_start();
        }
      },
      {
        name: '配信モードオフ', action: () => {
          SoundEffect.play(PresetSound.cardDraw);
          this.card.godeye_end();
        }
      },
      ContextMenuSeparator,
      {
